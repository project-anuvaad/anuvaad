...
def get_trans_user_data_from_db_daily_day_crn_1():
    ...
    # weekly_cron_file_name2 = config.WEEKLY_CRON_FILE_NAME2
    # file_save = str(filename)[:-10]+'_USER_WISE_JUD_Org_level_Statistics.csv'
    if os.path.exists(config.DOWNLOAD_FOLDER + "/" + daily_cron_file_name1):
        pass
    else:
        msg = generate_email_notification(
            users, "could not get the data files not found"
        ...
        send_email(msg)
        log_info(
            f"Generated alert email for daily cron (files not found),{daily_cron_file_name1} ",
            MODULE_CONTEXT,
        )
    user_docs = stats.get_all_users_active_inactive(usr_collection)
    log_info(
    ...
    try:
        ...
        for doc in user_docs:
            ...
            # )
        log_info(
            f"Data written into files {daily_cron_file_name1}",
            MODULE_CONTEXT,
        )
        return
    except Exception as e:
...
def get_trans_user_data_from_db_daily_day_crn_2():
    ...
    # weekly_cron_file_name2 = config.WEEKLY_CRON_FILE_NAME2
    # file_save = str(filename)[:-10]+'_USER_WISE_JUD_Org_level_Statistics.csv'
    if os.path.exists(config.DOWNLOAD_FOLDER + "/" + daily_cron_file_name2):
        pass
    else:
        msg = generate_email_notification(
            users, "could not get the data files not found"
        ...
        send_email(msg)
        log_info(
            f"Generated alert email for daily cron (files not found),{daily_cron_file_name2} ",
            MODULE_CONTEXT,
        )
    user_docs = stats.get_all_users_active_inactive(usr_collection)
    log_info(
    ...
    try:
        ...
        log_info(
            f"Data written into files {daily_cron_file_name2}",
            MODULE_CONTEXT,
        )
        return
    except Exception as e:
