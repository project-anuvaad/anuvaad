...
class UserEventView extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.eventData.length !== this.props.eventData.length && this.state.showLoader) {
    ...
    renderEventList = (s_id) => {
        const sentence_data = this.props.eventData.filter(data => data.s_id === s_id)
        return (
            <Tooltip title="View Events JSON" placement="right">
                <IconButton
                    ...
                    component="a"
                    onClick={() =>
                        this.handleEventListView(sentence_data[0].user_events)
                    }
                >
                    <ViewListIcon />
                </IconButton>
    ...
    handleEventListView = (events) => {
        if (events.length) {
            ...
            this.setState({ open: true, dialogMessage: "No user events are present", variant: "info" })
        }
        setTimeout(() => {
    ...
    renderJSONModalView = () => {
        return (
            ...
            >
                <JSONViewerModal copy={this.handleCopyJSON} close={this.handleModalClose} user_events={this.state.user_events} />
            </Modal>
        )
    }
    ...
    handleCopyJSON = () => {
        ...
        this.setState({ open: true, dialogMessage: "JSON Copied successfully...", variant: "success", openModal: false })
        setTimeout(() => {
            this.setState({ open: false, dialogMessage: "", variant: "info" })
        }, 3000)
    }
                ...
                options: {
                    ...
                    customBodyRender: (value, tableMeta, updateValue) => {
                        if (tableMeta.rowData) {
                            return (
                                <div>
                                    {this.renderEventList(tableMeta.rowData[5])}
                                </div>
                            );
                        }
                    },
