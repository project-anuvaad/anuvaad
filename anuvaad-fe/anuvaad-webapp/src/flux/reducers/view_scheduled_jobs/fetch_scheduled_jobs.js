...
function get_document_details(input) {
  ...
  input['jobs'].forEach(job => {
    let document = {}
    let timelines = []
    document['filename'] = job['input']['files'][0]["fileInfo"]? job['input']['files'][0]["fileInfo"]['name'] : ""
    ...
    if (langCodes && Array.isArray(langCodes) && langCodes.length > 0) {
      langCodes.map(lang => {
        if (lang.language_code === job['input']['files'][0]['sourceLanguage']) {
          sourceLang = lang.language_name
        }
        if (lang.language_code === job['input']['files'][0]['targetLanguage']) {
          targetLang = lang.language_name
        }
        return null
      })
    }
    document['source_language_code'] = sourceLang;
    document['target_language_code'] = targetLang;
...
export default function (state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case C.FETCH_SCHEDULED_NEXTPAGE: {
      let data = action.payload;
      let documents = get_document_details(data)
      return {
        ...state,
