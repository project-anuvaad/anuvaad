...
model_primalinenet = lp.Detectron2LayoutModel(LINE_LAYOUT_CONFIG_PATH, model_path=LINE_LAYOUT_MODEL_PATH, label_map={
                                              0: "LineRegion"}, extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", LINE_PRIMA_SCORE_THRESH_TEST])
...
class PRIMA(object):
        ...
        if len(boxes_final) != 0:
            for box in boxes_final:
                ...
                for idx, coord in enumerate(coords):
                    ...
                    vertical_dis = min(top_dis, bottom_dis,
                                       top_dis1, bottom_dis1)
                    horizon_dis = min(left_dis, right_dis,
                                      left_dis1, right_dis1)
                ...
                if abs(vertical_min_dis) < 150:
                    coords[ver_index][0] = int(
                        min(ver_coord_update[0], box[0]))
                    coords[ver_index][1] = int(
                        min(ver_coord_update[1], box[1]))
                    coords[ver_index][2] = int(
                        max(ver_coord_update[2], box[2]))
                    coords[ver_index][3] = int(
                        max(ver_coord_update[3], box[3]))
    ...
    def prima_craft_refinement(self, coords, boxes, tag_final, score):
        ...
        for idx1, coord1 in enumerate(boxes):
            ...
            for idx2, coord2 in enumerate(org_coord):
                ...
                rb = Rectangle(int(coord2[0]), int(
                    coord2[1]), int(coord2[2]), int(coord2[3]))
                ar = self.area(ra, rb)
                ...
                if ar == None:
                    count = count+1
            if check == True:
                org_coord2[index][0] = int(
                    min(coord1[0], org_coord2[index][0]))
                org_coord2[index][1] = int(
                    min(coord1[1], org_coord2[index][1]))
                org_coord2[index][2] = int(
                    max(coord1[2], org_coord2[index][2]))
                org_coord2[index][3] = int(
                    max(coord1[3], org_coord2[index][3]))
        ...
        coords, layout_class, score = self.craft_refinement(
            boxes_final, coords, tag_final, score)
    ...
    def update_coord(self, reg1, reg2, clss):
        try:
            ...
            reg1["boundingBox"]["vertices"][1]['x'] = max(
                box1_right, box2_right)
            reg1["boundingBox"]["vertices"][1]['y'] = min(box1_top, box2_top)
            reg1["boundingBox"]["vertices"][2]['x'] = max(
                box1_right, box2_right)
            reg1["boundingBox"]["vertices"][2]['y'] = max(
                box1_bottom, box2_bottom)
            reg1["boundingBox"]["vertices"][3]['x'] = min(box1_left, box2_left)
            reg1["boundingBox"]["vertices"][3]['y'] = max(
                box1_bottom, box2_bottom)
            reg1['class'] = clss
            return reg1
        except:
    ...
    def is_connected(self, region1, region2):
        ...
        if region_poly and base_poly:
            ...
            superimpose = self.polygon_overlap(
                region_poly, base_poly, region1, region2)
