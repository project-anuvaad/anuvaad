...
###################################
from src.services.main import preprocess as Service
#####################################
...
class Response(object):
    ...
    def workflow_response(self, task_id, task_starttime, debug_flush=False):
        ...
        try:
            error_validator.wf_keyerror(
                jobid, workflow_id, tool_name, step_order)
            ...
            for i, item in enumerate(input_files):
                ...
                if debug_flush == False:
                    ...
                    response = Service(app_context=app_context,
                                       base_dir=config.download_folder)
                    ##############################
                    if response['code'] == 200:
                        output_filename_json = file_ops.writing_json_file(
                            i, response['rsp'], self.DOWNLOAD_FOLDER)
                        file_res = file_ops.one_filename_response(
                            output_filename_json)
                        output_file_response.append(file_res)
                        task_endtime = eval(
                            str(time.time()).replace('.', '')[0:13])
                        response_true = CustomResponse(
                            Status.SUCCESS.value, jobid, task_id)
                        response_success = response_true.success_response(
                            workflow_id, task_starttime, task_endtime, tool_name, step_order, output_file_response)
                        response = copy.deepcopy(response_success)
                        log_info("successfully generated response for workflow",
                                 app_context.application_context)

                        return response
                    else:
                        post_error_wf(
                            response['code'], response['message'], app_context.application_context, None)
                        return None
                else:
                    log_info('flushing queue data, not handling file {}'.format(
                        input_files), app_context.application_context)
                    post_error_wf(400, 'flushing queue data, not handling file {}'.format(
                        input_files), app_context.application_context, None)
                    return None
            ...
            response = file_ops.error_handler(
                response_custom.status_code, "WORKFLOWKEY-ERROR", True)
            log_exception("workflow_response workflow key error: key value missing",
            ...
            response = file_ops.error_handler(
                response_custom.status_code, e.code, True)
            log_exception("workflow_response some error occured while validating file",
                          app_context.application_context, e)
            response = copy.deepcopy(response)
            return response
            ...
            response_custom.status_code['message'] = str(e)
            response = file_ops.error_handler(
                response_custom.status_code, "SERVICE_ERROR", True)
            log_exception("workflow_response Something went wrong during layout conversion.",
                          app_context.application_context, e)
            response = copy.deepcopy(response)
            return response
    ...
    def nonwf_response(self):
        ...
        try:
            ...
            output_filename_json = file_ops.writing_json_file(
                0, output_json_data, self.DOWNLOAD_FOLDER)
            response_true = file_ops.one_filename_response(
                output_filename_json)

            log_info("non workflow_response successfully generated response for rest server",
                     app_context.application_context)
            ...
            log_exception("non workflow_response some error occured while validating file",
                          app_context.application_context, e)
            response = copy.deepcopy(response)
            return response
        except ServiceError as e:
            ...
            response = file_ops.error_handler(
                response_custom, "SERVICE_ERROR", False)
            log_exception("non workflow_response Something went wrong during layout conversion.",
                          app_context.application_context, e)
            response = copy.deepcopy(response)
            return response

    def multi_thred_pre_processor(self, task_id, task_starttime, jobid):
        thread = threading.current_thread().name
        log_info("multi_thred_pre_processor" + str(thread) +
                 " | block-merger process started ===>", app_context.application_context)
        ...
        if "errorID" not in file_value_response.keys():
            ...
            log_info("process_merger_kf error send to error handler",
                     app_context.application_context)
