...
def draw_box(layout,df):
    ...
    boxes,coords,layout_class = sort(final_box,final_tag,df)  
...
def craft_refinement(coords,df,tag_final):
	coords_update = []
	org_coord = coords
	org_coord2= coords
	boxes = df_to_wordbox(df)
	boxes_final = []
...
def sort(final_box,final_tag,df):
	...
	boxes,coords,layout_class = craft_refinement(final_coord,df,final_tags)
	
	
	return boxes, coords,layout_class
		

def predict_primanet(image,in_df):
	#image1  = cv2.imread(image_path)
	image = image[..., ::-1] 
	layout = model_primalaynet.detect(image)
	boxes,coords,layout_class = draw_box(layout,in_df)
	final_coord= []
	for idx,coord in enumerate(coords):
		temp_dict={}
