...
def draw_box(layout,craft_coords):
    ...
    boxes,coords,layout_class = sort(final_box,final_tag,craft_coords)  
...
def remove_overlap(coords,tags_final):
	coord_update = coords
...
def craft_refinement(coords,craft_coords,tag_final):
	coords_update = []
	org_coord = coords
	org_coord2= coords
	boxes = craft_coords
	...
	coords, layout_class = remove_overlap(org_coord2,tags_final)
	if len(boxes_final)!=0:
		for box in boxes_final:
			...
			if abs(vertical_min_dis)<150 :
				...
				coords.append(box)
	coords,layout_class = remove_overlap(coords,layout_class)
	return boxes,coords	,layout_class
				
def sort(final_box,final_tag,craft_coords):
	...
	boxes,coords,layout_class = craft_refinement(final_coord,craft_coords,final_tags)
	
	
	return boxes, coords,layout_class
		

def predict_primanet(image,craft_coords):
	image1  = cv2.imread(image)
	image = image[..., ::-1] 
	layout = model_primalaynet.detect(image)
	boxes,coords,layout_class = draw_box(layout,craft_coords)
	final_coord= []
	for idx,coord in enumerate(coords):
		temp_dict={}
		temp_dict['class'] = layout_class[idx]
