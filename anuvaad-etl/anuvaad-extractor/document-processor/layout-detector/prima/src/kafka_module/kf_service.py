...
def process_module_name_kf():
    ...
    try:
        ...
        log_info("process_module_name_kf : trying to receive value from consumer ", LOG_WITHOUT_CONTEXT)
        
        for msg in consumer:
            if Consumer.get_json_data(msg.value) == None:
                log_info('process_module_name_kf - received invalid data {}'.format(msg.value), None)
                continue
            data            = Consumer.get_json_data(msg.value)
            jobid           = data['jobID']
            log_info('process_module_name_kf - received message from kafka, dumping into internal queue', data)
            ...
            ####################################
            Queue.put(data)
            log_info('process_module_name_kf - request in internal queue {}'.format(Queue.qsize()),
                        data)
            ########################################
            # else:
        ...
        log_exception("process_module_name_kf : Consumer didn't instantiate", None, e)
    except KafkaProducerError as e:
        response_custom = {}
        response_custom['message'] = e.message      
        file_ops.error_handler(response_custom, "KAFKA_PRODUCER_ERROR", True)
        log_exception("process_module_name_kf : response send to topic %s"%(config.output_topic), None, e)

def module_name_request_worker():
    file_ops            = FileOperation()
    DOWNLOAD_FOLDER     = file_ops.create_file_download_dir(config.download_folder)
    producer_tok        = Producer(config.bootstrap_server)
    log_info("module_name_request_worker : starting thread ", LOG_WITHOUT_CONTEXT)

    while True:
        data            = Queue.get(block=True)
        #################
        task_id         = str("Module_name" + str(time.time()).replace('.', ''))
        ###################
        task_starttime  = str(time.time()).replace('.', '')
        input_files, workflow_id, jobid, tool_name, step_order = file_ops.json_input_format(data)
        
        log_info("module_name_request_worker processing -- received message "+str(jobid), data)
        ...
        try:
            ...
            if file_value_response != None:
                if "errorID" not in file_value_response.keys():
                    ...
                    log_info("module_name_request_worker : response send to topic %s"%(config.output_topic), LOG_WITHOUT_CONTEXT)
                else:
                    log_info("module_name_request_worker : error send to error handler", data)

            log_info('module_name_request_worker - request in internal queue {}'.format(Queue.qsize()), data)

            Queue.task_done()
        except Exception as e:
            log_exception("module_name_request_worker ",  LOG_WITHOUT_CONTEXT, e)
