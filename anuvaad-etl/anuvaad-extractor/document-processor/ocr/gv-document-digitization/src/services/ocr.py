...
def get_text(path,page_dict,font_info):
    ...
    page_output,page_words = get_document_bounds(response.full_text_annotation,page_dict,font_info)
...
def text_extraction(file_properties,image_paths,file):
    ...
    file_properties.set_regions(page_res)
...
def avg_word_sep(words_lis):
        total_words = len(words_lis.words)
        height      = 0
        for word in words_lis.words:
...
def mask_image(path, page_dict,page_index,file_properties,image_width,image_height,margin= 0 ,fill=255):
    try:
        ...
        print('Service google vision digitization Error in masking out image {}'.format(e))
        return None
