                ...
                if region['children'] != None:
                    ...
                    for idx, line in enumerate(region['children']):
                        ...
                        if idx < len(region['children'])-1:
                            next_line_top = keys.get_top(
                                region['children'][idx+1])
                            ver_dis = abs(next_line_top-current_line_top)
                            max_height = max(keys.get_height(
                                region['children'][idx+1]), keys.get_height(region['children'][idx]))
                ...
                if line['children'] != None:
                    ...
                    for idx, word in enumerate(line['children']):
                        if idx < len(line['children']) - 1:
                            next_line_left = keys.get_left(
                                line['children'][idx + 1])
                            current_line_right = keys.get_right(
                                line['children'][idx])
...
class Region_Unifier:
        ...
        for idx, table in enumerate(tables):
            if 'regions' in table.keys():
                for idx2, cell in enumerate(table['regions']):
                    ...
                    del tmp_cells[idx2]
                    for word in words:
                        region_poly = get_polygon(word['boundingBox'])
                        base_poly = get_polygon(cell['boundingBox'])
                        area = 0
        ...
        for idx, v_block in enumerate(v_list):
            ...
            if 'regions' in v_block.keys():
                if v_block['regions'] != None and len(v_block['regions']) > 1:
                    ...
                    avrage_region_ver_ratio = avg__region_ver_dist / \
                        max(1, avg__region_height)
            else:
                log_info('region key not found for {}'.format(
                    v_block), app_context.application_context)
    ...
    def region_unifier(self, idx2, file, page_lines, page_regions, path):
        try:
            ...
            log_exception("Error occured during block unifier",
