...
def tess_eval(input):
    ...
    words  =   get_tess_text(image_crop,lang,mode_height,left,top,cls ,c_x,c_y,lang_detected)
...
def add_lines_to_tess_queue(page_lines,queue,lang,image,mode_height,rgn_idx,line_idx,cls,reg_left,reg_right,lang_detected):
    for index,coord in enumerate(page_lines):
        image_crop,c_x,c_y = crop_region(coord,image,cls,reg_left,reg_right)
        ...
        if image_crop is not None and image_crop.shape[1] >3 and image_crop.shape[0] > 3:
            line_meta_data = [lang, image_crop,coord,mode_height,left,top,rgn_idx,line_idx,cls,c_x,c_y,lang_detected]
