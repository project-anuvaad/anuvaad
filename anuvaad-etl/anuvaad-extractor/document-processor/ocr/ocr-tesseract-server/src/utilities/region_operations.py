...
def add_font(regions):
    ...
    return regions
def get_children_tree(lines,child_class,add_font,idx):
...
def collate_regions(regions, lines, child_class=None, grand_children=False,region_flag = True,skip_enpty_children=False,add_font=True ,child_key=False):
    if child_key:
        child_key = "LINES"
    else:
        child_key='regions'
    idx = index.Index();  lines_intersected = []
    if regions !=None and len(regions) > 0:
        lines_intersected =[]
        idx,lines = get_children_tree(lines,child_class,add_font,idx)
        for region_index, region in enumerate(regions):
            region_poly = get_polygon(region['boundingBox']);  children_lines =[]
            if region_poly:
...
def collate_cell_regions(regions, lines, child_class=None, grand_children=False,region_flag = True,skip_enpty_children=False,add_font=True ):

    child_key='regions';  idx = index.Index();  lines_intersected = []
    if regions !=None and len(regions) > 0:
        idx,lines = get_children_tree(lines,child_class,add_font,idx)
        for region_index, region in enumerate(regions):
            children_lines =[]
...
def get_avrage_size(regions):
    size = 0
    if regions != None:
        len_regions = len(regions);  count=0
        if len_regions> 0 :
            for region in regions :
                if 'font' in region.keys():
                    size += region['font']['size'];  count=count+1
            if count==0:
                count=1
