...
def create_pdf_processing_paths(filename, base_dir):
    ...
    working_dir = os.path.join(
        base_dir, os.path.splitext(filename)[0] + "_" + str(uuid.uuid4())
    )
    ret = create_directory(working_dir)

    if ret == False:
        log_error(
            "unable to create working directory {}".format(working_dir),
            app_context.application_context,
            None,
        )
        return None, False
    ...
    log_info(
        "created processing directories successfully {}".format(working_dir),
        app_context.application_context,
    )
...
def extract_image_paths_from_pdf(filepath, workspace_output_dir):
    ...
    paths = pdf2image.convert_from_path(
        filepath,
        dpi=config.EXRACTION_RESOLUTION,
...
def extract_pdf_metadata(filename, working_dir, base_dir):
    ...
    log_info(
        "filepath {}, working_dir {}".format(pdf_filepath, working_dir),
        app_context.application_context,
    )
    try:
        ...
        log_error(
            "error in extracting images from {}".format(pdf_filepath),
            app_context.application_context,
            e,
        )
        return None
    ...
    log_info(
        "Extraction of images from {} completed in {}".format(
            pdf_filepath, extraction_time
        ),
        app_context.application_context,
    )
