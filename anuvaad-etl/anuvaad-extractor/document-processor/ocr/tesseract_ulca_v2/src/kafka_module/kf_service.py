...
def consumer_validator():
    try:
        ...
        log_exception(
            "consumer_validator : error in kafka opertation while listening to consumer on topic %s"
            % (config.input_topic),
            None,
            None,
        )
...
def push_output(producer, topic_name, output, jobid, task_id, data):
    try:
        ...
        log_info(
            "push_output : producer flushed value on topic %s" % (topic_name), data
        )
    except Exception as e:
        response_custom = CustomResponse(Status.ERR_STATUS.value, jobid, task_id)
        log_exception(
            "push_output : Response can't be pushed to queue %s" % (topic_name),
            data,
            None,
        )
...
def process_vision_ocr_kf():
    ...
    try:
        ...
        log_info(
            "process_ocr_gv_15_kf : trying to receive value from consumer ",
            LOG_WITHOUT_CONTEXT,
        )
        ...
        while True:
            ...
            for msg in consumer:
                ...
                if Consumer.get_json_data(msg.value) == None:
                    log_info(
                        "process_ocr_gv_15_kf - received invalid data {}".format(
                            msg.value
                        ),
                        None,
                    )
                ...
                jobid = data["jobID"]
                log_info(
                    "process_ocr_gv_15_kf - received message from kafka, dumping into internal queue",
                    data,
                )
                (
                    input_files,
                    ...
                    tool_name,
                    step_order,
                ) = file_ops.json_input_format(data)
        ...
        log_exception(
            "process_vision_ocr_kf : response send to topic %s" % (config.output_topic),
            None,
            e,
        )
...
def vision_ocr_request_worker():
    ...
    while True:
        ...
        log_info(
            "vision_ocr_request_worker processing -- received message " + str(jobid),
            data,
        )
        ...
        try:
            ...
            file_value_response, gv_file_response = response_gen.workflow_response(
            ...
            if file_value_response != None:
                if "errorID" not in file_value_response.keys():
                    ...
                    log_info(
                        "process_ocr_gv_15_kf - message dumped :  {}".format(
                            file_value_response
                        ),
                        data,
                    )
                    log_info(
                        "vision_ocr_request_worker : response send to topic %s"
                        % (config.output_topic),
                        LOG_WITHOUT_CONTEXT,
                    )
                else:
                    log_info(
                        "vision_ocr_request_worker : error send to error handler", data
                    )

            log_info(
                "vision_ocr_request_worker - request in internal queue {}".format(
                    Queue.qsize()
                ),
                data,
            )
