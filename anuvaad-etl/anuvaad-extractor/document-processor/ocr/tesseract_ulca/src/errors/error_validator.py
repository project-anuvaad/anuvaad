    ...
    def wf_keyerror(self, jobid, workflow_id, tool_name, step_order):
        if "" in (jobid, workflow_id, tool_name, step_order):
            raise WorkflowkeyError(
                400, "jobID/workflowCode/tool/stepOrder is missing in input json"
            )
        elif None in (jobid, workflow_id, tool_name, step_order):
            raise WorkflowkeyError(
                400, "jobID/workflowCode/tool/stepOrder somehow got None value"
            )
        ...
        if len(input_files) == 0 or not isinstance(input_files, list):
            ...
            for i, item in enumerate(input_files):
                ...
                if input_filename == "" or input_filename is None:
                    raise FileErrors(
                        "FILENAME_ERROR", "Filename not found or its empty"
                    )
                ...
                if in_file_type == "" or in_file_type is None:
                    raise FileErrors(
                        "FILE_TYPE_ERROR",
                        "This file type is not allowed. Currently, support only pdf file.",
                    )
                # elif file_ops.check_file_extension(in_file_type) is False:
                # raise FileErrors("FILE_TYPE_ERROR", "This file type is not allowed. Currently, support only pdf file.")
                elif (
                    file_ops.check_path_exists(input_filepath) is False
                    or file_ops.check_path_exists(self.DOWNLOAD_FOLDER) is False
                ):
                    raise FileErrors(
                        "DIRECTORY_ERROR", "There is no input/output Directory."
                    )
                elif identifier == "" or identifier is None:
                    raise FileErrors(
                        "LOCALE_ERROR",
                        "No language input or unsupported language input.",
                    )
