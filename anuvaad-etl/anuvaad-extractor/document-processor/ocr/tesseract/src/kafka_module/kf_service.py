...
def process_tesseract_ocr_kf():
    ...
    try:
        ...
        log_info("process_tesseract_ocr_kf : trying to receive value from consumer ", LOG_WITHOUT_CONTEXT)
        
        for msg in consumer:
            if Consumer.get_json_data(msg.value) == None:
                log_info('process_tesseract_ocr_kf - received invalid data {}'.format(msg.value), None)
                continue
            data            = Consumer.get_json_data(msg.value)
            jobid           = data['jobID']
            log_info('process_tesseract_ocr_kf - received message from kafka, dumping into internal queue', data)
            ...
            ####################################
            Queue.put(data)
            log_info('process_tesseract_ocr_kf - request in internal queue {}'.format(Queue.qsize()),
                        data)
            ########################################
            # else:
