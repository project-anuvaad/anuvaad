...
from src.services.tesseract import tess_text_extraction
from src.utilities.request_parse import get_pageinfo,get_files, File,get_json,get_ocr_config
...
def frequent_height(page_info):
    ...
    if len(page_info) > 0 :
        ...
        return  0
def get_text(file_properties,page_index,page_path,page_dict,ocr_level, lang):
    
    #path = config.BASE_DIR+path
    img = cv2.imread(path)
    font_info = file_properties.get_fontinfo(page_index)
    ...
    page_c_words = file_properties.get_words(page_index)
    width, height = file_properties.get_pageinfo(0)
    tess_page_words = []
    if config.OCR_LEVEL[ocr_level] == 'words':
        for word_index, word in enumerate(page_c_words):
            if config.IS_DYNAMIC: 
                region_words_org = coord_adjustment(page_path, copy.deepcopy(region_words))
                region_ocr,tess_coord = tess_text_extraction(lang, page_path, region_words_org,region_words, width, height,mode_height)
                tess_page_words.extend(tess_coord)
            else:
                region_ocr,tess_coord = tess_text_extraction(lang, page_path, region_words,region_words, width, height,mode_height)
                tess_page_words.extend(tess_coord)
...
def text_extraction(file_properties,image_paths,file,ocr_level, lang):
    page_res = []
    width, height = file_properties.get_pageinfo(0)
    for idx,image_path in enumerate(image_paths):
        ...
        page_output,page_words = get_text(file_properties,idx,image_path,page_dict,ocr_level, lang)
