...
def bound_coordinate(coordinate,max):
    if coordinate < 0 :
        coordinate = 0
    if coordinate > max:
        coordinate = max - 2
...
def frequent_height(page_info):
    ...
    if len(page_info) > 0 :
        ...
        return  0
def tess_text_extraction(lang, page_path, regions,region_org,width, height,mode_height):

    #freq_height = frequent_height(regions)
    #for idx, level in enumerate(regions):
    coord = get_coord(regions)
    if len(coord)!=0 and abs(coord[3] - coord[1]) > config.REJECT_FILTER :
        ...
        region_org['text'] = text
        #region_org['regions'] = tess_coord
