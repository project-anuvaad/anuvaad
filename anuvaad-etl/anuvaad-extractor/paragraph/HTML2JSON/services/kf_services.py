...
def process_HTML_kf():
    ...
    try:
        ...
        for msg in consumer:
            ...
            producer_html2json = Producer(config.bootstrap_server) 
            producer = producer_html2json.producer_fn()
            producer.send(config.html_output_topic, value = response.status_code)
            producer.flush()
            log.error("error in kafka opertation producer flushed value on topic %s"%(config.html_output_topic))
