...
class AnuvaadOdiaTokenizer(object):
    """
    Default abbrevations
    incomplete char range = ([\u0980-\u0983,\u09BC-\u09CD,\u09D7,\u09E2-\u09E3, \u09FA-\u09FE])
    complete char range = ([\u0B05-\u0B39,\u00B71,\u0B5C-\u0B61])
    ...
    def tokenize(self, text):
        ...
        output = sentence_tokenize.sentence_split(text, lang='or')
...
#         patterns = re.findall(
#             r'(?:(?:(?:[ ][(]?(?:(?:[0,9]|[i]|[x]|[v]){1,3}|[a-zA-Z\u0980-\u09FE]{1,1})[)])|(?:[ ](?:(?:[0-9]|[i]|[x]|[v]){1,3}|[a-zA-Z\u0980-\u09FE]{1,1})[.][ ])))',
#             text)
#         index = 0
#         if patterns is not None and isinstance(patterns, list):
#             for pattern in patterns:
...
#         patterns = re.findall(r'[0-9]{1,4}[.][0-9]{1,2}[.][0-9]{1,4}',
#                               text)  # [0-9]{,2}[.][0-9]{,2}[.][0-9]{2,4}   [0-9]{1,4}[.][0-9]{1,2}[.][0-9]{1,4}
#         index = 0
#         if patterns is not None and isinstance(patterns, list):
#             for pattern in patterns:
#                 pattern_obj = re.compile(re.escape(pattern))
...
#         index = 0
#         if self._dot_with_beginning_number_abbrevations is not None and isinstance(
#                 self._dot_with_beginning_number_abbrevations, list):
...
#             index_no_gen += 1
#         if self._text_abbrevations_without_space is not None and isinstance(self._text_abbrevations_without_space,
#                                                                             list):
