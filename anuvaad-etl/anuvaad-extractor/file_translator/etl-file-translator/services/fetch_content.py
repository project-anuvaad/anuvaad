...
class FetchContent(object):
    ...
    def store_reference_link(self, job_id='', location=''):
        ...
        log_info(f'Store Reference Link STARTED for job id: {job_id}', self.json_data)
        ...
        log_info(f'Store Reference Link ENDED for job id: {job_id}', self.json_data)

        if rspn.status_code not in [200, 201]:
            log_info('Response:: %s' % rspn.text, self.json_data)
