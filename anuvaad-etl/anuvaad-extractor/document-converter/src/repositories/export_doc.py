...
class DocumentExporterRepository(object):
    ...
    def get_data_from_ocr_content_handler(self, record_id, start_page=0, end_page=0):
        ...
        try:
            ...
            # request_url = doc_utils.generate_url(config.OCR_CONTENT_HANDLER_ENDPOINT, record_id, start_page, end_page)
            request_url='https://auth.anuvaad.org/anuvaad/ocr-content-handler/v0/ocr/fetch-document?start_page=1&end_page=1&recordID=122'
    ...
    def create_pdf_to_text(self,pdf_filepath):
        ...
        text_filename = os.path.splitext(os.path.basename(output_filepath))[0] + '.txt'
        text_filepath = os.path.join(os.path.dirname(output_filepath), text_filename)
