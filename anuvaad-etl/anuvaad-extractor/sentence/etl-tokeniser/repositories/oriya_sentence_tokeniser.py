...
"""
class AnuvaadOriyaTokenizer(object):
    """
    Default abbrevations
    incomplete char range = ([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])
    complete char range = ([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])    
    number range = ([\u0B66-\u0B6F])
    source for unicodes : https://unicode.org/charts/PDF/U0B00.pdf
    """
    _abbrevations_with_non_generalize_pattern = []
    _abbrevations_with_non_generalize = []
    _text_abbrevations_pattern_cic = r'((\s)(([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])?([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])?(\u002e)(\s)?){1,})'
    _text_abbrevations_pattern_cci = r'((\s)(([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])?([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])?(\u002e)(\s)?){1,})'
    _text_colon_abbreviations_pattern = r'(([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])?[:](\s)?[\u0B01-\u0B77|-])'
    _text_abbrevations_without_space_pattern = r'(^(([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])?([\u0B05-\u0B39,\u0B5C-\u0B61,\u0B71])?([\u0B01-\u0B03,\u0B3C-\u0B57,\u0B62-\u0B65,\u0B70,\u0B72-\u0B77])?(\u002e)(\s)?){1,})'
